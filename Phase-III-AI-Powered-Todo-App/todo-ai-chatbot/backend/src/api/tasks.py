...
async def get_tasks_endpoint(
    ...
    try:
        ...
        result = list_tasks(session=session, user_id=current_user.id, completed=None)
...
async def toggle_task_endpoint(
    ...
    try:
        ...
        task = session.exec(select(Task).where(Task.user_id == current_user.id, Task.id == task_id_int)).first()
