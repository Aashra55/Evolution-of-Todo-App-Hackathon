            ...
            {pendingTasks.map(task => (
              <li key={task.id} className="p-4 task-item task-item-pending border-l-4 border-blue-500 bg-blue-50">
                <div className="flex items-start">
                  <span className="inline-block w-5 h-5 border-2 border-blue-500 rounded mr-3 mt-0.5 flex-shrink-0"></span>
          ...
          <ul className="space-y-2">
            {completedTasks.map(task => (
              <li key={task.id} className="p-4 task-item task-item-completed border-l-4 border-green-500 bg-green-50 opacity-75">
                <div className="flex items-start">
                  <span className="inline-block w-5 h-5 bg-green-500 rounded mr-3 mt-0.5 flex-shrink-0 flex items-center justify-center">
